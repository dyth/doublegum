...
def plot_sample_efficiency(ax, frames, point_estimates, interval_estimates, algorithms=None):
    ...
    for algorithm in algorithms[1:] + algorithms[:1]:
        ...
        ax.plot(
            ...
            linewidth=2 if algorithm == r'\textbf{Pessimism 1 (Default)}' else 1.2,
            label=algorithm,
        )
        phi = (np.sqrt(5) - 1) / 2
...
# fig, axes = plt.subplots(height, width, figsize=(8, 8.25))
fig, axes = plt.subplots(height, width, figsize=(22, 4.75))
...
for algorithm in algorithms:
    plt.plot(0, 0, color=colors[algorithm], label=algorithm)
# leg = plt.legend(loc='upper center', ncol=2, fontsize=30, bbox_to_anchor=(0.5, -0.25))
# leg = plt.legend(loc='upper center', ncol=3, fontsize=30, bbox_to_anchor=(0.5, -.25))
leg = plt.legend(ncol=1, fontsize=30, loc='center left', bbox_to_anchor=(1, 0.5))
